...
check = () => {
  ...
  //checks through the Input String and removes all duplicates, retaining a single occurrence of such duplicated word
  for (let i = 0; i < input_string_array.length; i++) {
    if (duplicates.search(input_string_array[i]) == -1) {
      if (
        input_string.lastIndexOf(input_string_array[i]) ==
        input_string.indexOf(input_string_array[i])
      ) {
...
sort_words = (duplicate_void) => {
  ...
  const duplicate_void_array = duplicate_void.split(" ");
    ...
    for (let j = i + 1; j < array_length; j++) {
      ...
      if (duplicate_void_array[i].length === duplicate_void_array[j].length) {
        ...
        for (let k = 0; k < duplicate_void_array[i].length; k++) {
          if ((
            duplicate_void_array[j].indexOf(
              duplicate_void_array[i].charAt(k)
            ) !== -1) && (
              duplicate_void_array[i].indexOf(
                duplicate_void_array[j].charAt(k)
              ) !== -1)
          ) {
            count++;
          } else count = 0;
          ...
          if (count === duplicate_void_array[i].length) {
            same_letters +=
              `<p> <strong>
                ${duplicate_void_array[i]}
                </strong> \xa0 and \xa0 <strong>
                ${duplicate_void_array[j]}
                </strong> \xa0  contains the same letters </p>`;
          }
        }
        count = 0
      }
  ...
  if (same_letters === "") {
    same_letters =
      " <P> There are no Words in This sentence Containing the same letters </p>";
  }
